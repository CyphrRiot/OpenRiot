...
**Release Path Checker** — The v7.5 notes said `sysupgrade -R 7.9` was
"unlikely to work" from snapshots, but we only documented the failure
case. The success case is real: pre-release snapshots (built before the
release date) can safely migrate to stable with `sysupgrade -R 7.9`. The
new `openriot --check-release-path` command parses your kernel build date,
compares it against the release date, and tells you exactly which camp
you are in: pre-release (run the commands) or post-release (stay on
-current or fresh install). The README now has a decision tree table under
"Migrating from -current to Stable Release" that makes the choice
obvious.
...
| | `--wifi-click`, `--check-release-path` commands |
| `source/commands/helpers.go` | `checkReleasePath()`: kernel date vs |
...
| `README.md` | App launcher: Monitor Resolution; polybar `·` separator |
| | Migration decision tree; `--check-release-path` docs |
